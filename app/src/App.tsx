    ...
    <div className="topbar">
      {!online && (
        <span className={`chip ${offlineReady ? "chip-ok" : "chip-warn"}`}>
          {offlineReady
            ? "Offline · charts ready"
            : "Offline · charts not downloaded"}
        </span>
      )}
      <ForecastAgeChip />
      <ShiftChip />
      {gpsStatus === "acquiring" && (
        <span className="chip">Acquiring GPS…</span>
      )}
      {gpsStatus === "denied" && (
        <span className="chip chip-warn">
          Location denied — allow location for this site
        </span>
      )}
      {gpsStatus === "error" && (
  ...
  return (
    <div className="fabstack">
      ...
      <button
        ...
        onClick={() =>
          follow ? useAppStore.getState().setFollow(false) : locateAndFollow()
        }
...
export default function App() {
  ...
  const routing =
    destination != null && tripStartedAt == null && sheetTab == null;
